...
import { Search, Plus, Filter, X, AlertCircle, Inbox } from 'lucide-react'
...
const Home = () => {
  ...
  // ✅ Handle all possible response shapes + treat errors as empty
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        {isAuthenticated && !user?.organizationId && (
          <div className="mb-8 bg-blue-50/95 backdrop-blur-md border-2 border-blue-200 rounded-xl p-6 shadow-xl">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              <div>
                <h3 className="text-lg font-bold text-gray-900 mb-2">Are you an organization?</h3>
                <p className="text-gray-700 font-medium">
                  Create your organization profile and start posting volunteer opportunities
                </p>
              </div>
              <Link to="/create-organization"
                className="inline-flex items-center space-x-2 bg-emerald-600 text-white px-6 py-3 rounded-lg hover:bg-emerald-700 font-bold whitespace-nowrap shadow-lg hover:scale-105 transition-all">
        ...
        {hasActiveFilters && (
          <div className="mb-6 flex flex-wrap gap-2 items-center bg-white/90 backdrop-blur-md p-4 rounded-lg shadow-lg">
            ...
            <button onClick={clearFilters} className="text-sm text-gray-700 hover:text-gray-900 underline font-medium">
              Clear all
            </button>
          </div>
        )}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 bg-white/95 backdrop-blur-md rounded-xl shadow-2xl p-8">
            <AlertCircle className="w-14 h-14 text-red-400 mx-auto mb-4" />
            <p className="text-gray-800 text-lg font-bold mb-2">Could not load opportunities</p>
            <p className="text-gray-500 text-sm mb-6">There was a problem connecting to the server.</p>
            <button onClick={() => refetch()}
              className="bg-emerald-600 text-white px-6 py-3 rounded-lg hover:bg-emerald-700 font-bold shadow-lg transition-colors">
              Try Again
            </button>
          </div>

        ) : opportunities.length === 0 ? (
          // ✅ No opportunities in DB at all
          <div className="text-center py-16 bg-white/95 backdrop-blur-md rounded-xl shadow-2xl p-8">
            <Inbox className="w-14 h-14 text-gray-300 mx-auto mb-4" />
            <p className="text-gray-700 text-xl font-bold mb-2">No opportunities yet</p>
            <p className="text-gray-400 text-sm mb-6">
              Check back soon — new volunteer opportunities are added regularly.
            </p>
            {!isAuthenticated && (
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Link to="/register"
                  className="bg-emerald-600 text-white px-6 py-3 rounded-lg hover:bg-emerald-700 font-bold transition-colors">
                  Join as Volunteer
                </Link>
                <Link to="/login"
          ...
          // ✅ Opportunities exist but none match filters
          <div className="col-span-full text-center py-12 bg-white/95 backdrop-blur-md rounded-xl shadow-2xl p-8">
      ...
      {!isAuthenticated && (
        <div className="relative text-white py-20 px-4 mt-12">
          ...
          <div className="max-w-4xl mx-auto text-center relative z-10">
            <h2 className="text-4xl font-bold mb-4 drop-shadow-lg">Ready to Make a Difference?</h2>
