...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try { await loginMutation.mutateAsync(formData) } catch {}
  }
  ...
  return (
    <DynamicBackground category="minimal" overlay={0.8}>
      <div className="min-h-screen flex flex-col items-center justify-center py-8 px-4">
        ...
        <div className="w-full max-w-md bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl p-6 sm:p-8">
          <div className="text-center mb-6 sm:mb-8">
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">{t('login')}</h2>
            <p className="text-gray-600 font-medium text-sm sm:text-base">Welcome back! Sign in to continue</p>
          </div>

          <button onClick={handleGoogleLogin} type="button"
            className="w-full flex items-center justify-center space-x-3 bg-white border-2 border-gray-300 rounded-xl py-3 hover:bg-gray-50 transition-all mb-4">
            ...
            <span className="font-bold text-gray-700 text-sm sm:text-base">{t('googleLogin')}</span>
          </button>

          <div className="relative mb-4">
            <div className="absolute inset-0 flex items-center"><div className="w-full border-t-2 border-gray-200"></div></div>
            <div className="relative flex justify-center text-sm"><span className="px-4 bg-white text-gray-500 font-bold">OR</span></div>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div>
              ...
              <input type="email" name="email" value={formData.email} onChange={handleChange}
                ...
                className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all text-sm sm:text-base" />
            </div>
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} name="password" value={formData.password} onChange={handleChange}
                  ...
                  className="w-full px-4 py-3 pr-12 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all text-sm sm:text-base" />
            ...
            <button type="submit" disabled={loginMutation.isPending}
              className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 text-white py-3 rounded-xl hover:from-emerald-700 hover:to-teal-700 font-bold disabled:opacity-50 transition-all shadow-xl text-sm sm:text-base mt-2">
          ...
          <p className="mt-5 text-center text-sm text-gray-600 font-medium">
            ...
            <Link to="/register" className="text-emerald-600 hover:text-emerald-700 font-bold hover:underline">{t('register')}</Link>
          </p>

          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-xs sm:text-sm text-blue-800 text-center">
              <span className="font-semibold">📧 Note:</span> Verify your email before logging in.
            </p>
          </div>
        </div>
      </div>
