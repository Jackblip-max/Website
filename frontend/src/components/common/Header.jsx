          ...
          <div className="flex items-center space-x-4">
            ...
            {!isAuthenticated ? (
              <div className="flex items-center space-x-2">
                {/* Login goes straight to /login — no modal, no admin choice */}
                <Link
                  to="/login"
                ...
                {profileOpen && (
                  <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-xl border border-gray-200 py-2">
                    ...
                    <Link to="/profile" onClick={() => setProfileOpen(false)}
                      ...
                      <User className="w-4 h-4" /><span>{t('myProfile')}</span>
                    </Link>

                    {user?.organization && (
                      <Link to="/org-dashboard" onClick={() => setProfileOpen(false)}
                        className="w-full text-left px-4 py-2 hover:bg-gray-100 flex items-center space-x-2 text-gray-700">
                        <Building2 className="w-4 h-4" /><span>{t('myOrganization')}</span>
                      </Link>
                    )}

                    <Link to="/saved" onClick={() => setProfileOpen(false)}
                      className="w-full text-left px-4 py-2 hover:bg-gray-100 flex items-center space-x-2 text-gray-700">
                      <Bookmark className="w-4 h-4" /><span>{t('savedTasks')}</span>
                    </Link>

                    <Link to="/applications" onClick={() => setProfileOpen(false)}
                      className="w-full text-left px-4 py-2 hover:bg-gray-100 flex items-center space-x-2 text-gray-700">
                      <Clock className="w-4 h-4" /><span>{t('applications')}</span>
                    </Link>
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-gray-200 bg-white">
          <nav className="px-4 py-4 space-y-2">
            {[['/', t('home')], ['/about', t('about')], ['/categories', t('categories')], ['/how-it-works', t('howItWorks')]].map(([to, label]) => (
